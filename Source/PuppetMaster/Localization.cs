    ...
    public static class Localization
    {
        ...
        private static void InitializeEnglish()
        {
            Strings[PluginLanguage.English] = new Dictionary<string, string>
            {
                ...
                ["Common.Tooltip.AllowAllCommands"] = "Allow All Text Commands (Keyword Mode Only)\n\nWhen enabled:\n1. Keyword must match first (e.g., \"test\")\n2. Text extraction rules:\n   - Default: Extract first word/phrase after keyword\n   - With #: Extract everything between keyword and #\n3. Command formatting:\n   - Add / prefix if not present\n   - Keep existing / prefix if present\n4. Examples:\n   - \"test dance\" → /dance\n   - \"test, dance!\" → /dance (punctuation ignored)\n   - \"test ac Vercure <t>#heal me\" → /ac Vercure <t>\n   - \"test /ac Vercure\" → /ac Vercure (no double slash)\n5. Note: This option only appears in Keyword mode.",
        ...
        private static void InitializeChinese()
        {
            Strings[PluginLanguage.Chinese] = new Dictionary<string, string>
            {
                ...
                ["Common.Tooltip.AllowAllCommands"] = "允许所有文本指令（仅关键词模式）\n\n启用后：\n1. 首先必须匹配关键词（如\"test\"）\n2. 文本提取规则：\n   - 默认：提取关键词后的第一个词/短语\n   - 使用#时：提取关键词和#之间的所有内容\n3. 命令格式化：\n   - 如果没有/前缀则添加\n   - 如果已有/前缀则保留\n4. 示例：\n   - \"test dance\" → 执行 /dance\n   - \"test, dance!\" → 执行 /dance（标点被忽略）\n   - \"test ac Vercure <t>#治疗我\" → 执行 /ac Vercure <t>\n   - \"test /ac Vercure\" → 执行 /ac Vercure（不会添加双斜杠）\n5. 注意：此选项仅在关键词模式下显示。",
