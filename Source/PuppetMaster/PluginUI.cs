...
namespace PuppetMaster
{
    public class ConfigWindow : Window, IDisposable
    {
        ...
        private static void DrawCommandSettings()
        {
            ...
            // ========== Pokemon Mode Settings ==========
            ImGui.TextColored(ImGuiColors.DalamudYellow, Localization.Get("UI_PokemonSettings"));

            // Enable/Disable toggle
            var pokemonEnabled = Service.configuration.PokemonModeEnabled;
            if (ImGui.Checkbox(Localization.Get("Pokemon_Enable"), ref pokemonEnabled))
            {
                Service.configuration.PokemonModeEnabled = pokemonEnabled;
                if (!pokemonEnabled)
                {
                    // Clear active player when disabling
                    Service.configuration.PokemonActivePlayer = null;
                    Service.configuration.PokemonActivationTime = null;
                }
                Service.configuration.Save();
            }
            ...
            if (ImGui.IsItemHovered())
            {
                ImGui.SetTooltip(Localization.Get("Pokemon_Enable_Tooltip"));
            }
            ...
            if (Service.configuration.PokemonModeEnabled)
            {
                ...
                // Activation password
                ImGui.Text(Localization.Get("Pokemon_Password"));
                var password = Service.configuration.PokemonActivationPassword ?? "pokemon activate";
                if (ImGui.InputText("##PokemonPassword", ref password, 100))
                {
                    Service.configuration.PokemonActivationPassword = password;
                    Service.configuration.Save();
                }

                if (ImGui.IsItemHovered())
                {
                    ImGui.SetTooltip(Localization.Get("Pokemon_Password_Tooltip"));
                }

                // Timeout setting
                ImGui.Text(Localization.Get("Pokemon_Timeout"));
                var timeout = Service.configuration.PokemonTimeoutMinutes;
                if (ImGui.InputInt("##PokemonTimeout", ref timeout, 5, 10))
                {
                    Service.configuration.PokemonTimeoutMinutes = Math.Max(0, timeout);
                    Service.configuration.Save();
                }

                if (ImGui.IsItemHovered())
                {
                    ImGui.SetTooltip(Localization.Get("Pokemon_Timeout_Tooltip"));
                }

                ImGui.Spacing();

                // Current status
                ImGui.Text(Localization.Get("Pokemon_CurrentStatus"));
                ImGui.Indent(10);

                if (!string.IsNullOrEmpty(Service.configuration.PokemonActivePlayer))
                {
                    var playerName = Service.configuration.PokemonActivePlayer.Split('@')[0];
                    ImGui.TextColored(ImGuiColors.HealerGreen,
                        string.Format(Localization.Get("Pokemon_Status_Active"), playerName));

                    if (Service.configuration.PokemonActivationTime.HasValue)
                    {
                        var elapsed = DateTime.Now - Service.configuration.PokemonActivationTime.Value;
                        var remaining = TimeSpan.FromMinutes(Service.configuration.PokemonTimeoutMinutes) - elapsed;

                        if (remaining.TotalSeconds > 0 && Service.configuration.PokemonTimeoutMinutes > 0)
                        {
                            ImGui.Text(string.Format(Localization.Get("Pokemon_Status_TimeRemaining"),
                                (int)remaining.TotalMinutes, remaining.Seconds));
                        }
                        else if (Service.configuration.PokemonTimeoutMinutes > 0)
                        {
                            ImGui.TextColored(ImGuiColors.DalamudRed, Localization.Get("Pokemon_Status_Expired"));
                        }
                    }

                    // Deactivate button
                    if (ImGui.Button(Localization.Get("Pokemon_Deactivate")))
                    {
                        Service.configuration.PokemonActivePlayer = null;
                        Service.configuration.PokemonActivationTime = null;
                        Service.configuration.Save();
                ...
                {
                    ImGui.TextColored(ImGuiColors.DalamudGrey, Localization.Get("Pokemon_Status_Inactive"));
                }
                ...
                ImGui.Spacing();
                ImGui.Separator();
                ImGui.TextColored(ImGuiColors.DalamudGrey, Localization.Get("Pokemon_Instructions_Title"));
                ImGui.Text(Localization.Get("Pokemon_Instructions_Step1"));
                ImGui.Text(Localization.Get("Pokemon_Instructions_Step2"));
                ImGui.Text(Localization.Get("Pokemon_Instructions_Step3"));
                ImGui.TextColored(ImGuiColors.DalamudRed, Localization.Get("Pokemon_Instructions_Warning"));
            }
            else
            {
                ImGui.TextColored(ImGuiColors.DalamudGrey, Localization.Get("Pokemon_DisabledMessage"));
            }
            ...
            if (ImGui.BeginTabItem(Localization.Get("Tab.EditReaction")))
            {
                ...
                if (Service.IsValidReactionIndex(Service.configuration.CurrentReactionEdit))
                {
                    ...
                    if (ImGui.Combo("##TriggerMode", ref triggerMode, triggerModeNames, triggerModeNames.Length))
                    {
                        ...
                        if (reaction.TriggerMode == TriggerMode.Keyword &&
                        newTriggerMode != TriggerMode.Keyword &&
                    ...
                    // Only show AllowAllCommands for Keyword mode
                    if ((TriggerMode)triggerMode == TriggerMode.Keyword)
                    {
