                            ...
                            if (!useDefaultEnabledChannels)
                            {
                                ...
                                foreach ((int, int) range in ranges)
                                {
                                    for (int index = range.Item1; index < range.Item2; ++index)
                                    {
                                        ...
                                        bool enabled = selectedWhitelistedPlayer.EnabledChannels[index].Enabled;

                                        if (ImGui.Checkbox(selectedWhitelistedPlayer.EnabledChannels[index].Name, ref enabled))
                                        {
                                            selectedWhitelistedPlayer.EnabledChannels[index].Enabled = enabled;
                                            Service.configuration.Save();
                                        }
                                    }
                                }
